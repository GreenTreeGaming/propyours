    ...
    return (
        <div className="pt-24 min-h-screen bg-white font-body relative overflow-hidden">
                    ...
                    {ownerPlans.map((plan, index) => {
                        ...
                        return (
                            <motion.div
                                ...
                                className={`flex flex-col relative rounded-3xl transition-all duration-300 border-2 overflow-hidden bg-white ${isPopular
                            ...
                            >
                                ...
                                {/* Card Ribbon Badge */}
                                {plan.badge && (
                                    <div className={`absolute top-0 right-0 py-1.5 px-6 rounded-bl-2xl text-[10px] font-black uppercase tracking-wider ${isPopular
                                ...
                                <div className="p-8 pb-6 border-b border-gray-100 flex-none relative">
                                    {/* Dark Card Subtle Overlay */}
                                    {isPremium && (
                                        <div className="absolute top-0 right-0 w-32 h-32 bg-primary/10 rounded-full blur-2xl pointer-events-none" />
                                    )}

                                    <div className="flex items-center gap-3 mb-4">
                                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${isPremium
                                            ? "bg-zinc-800"
                                            : isPopular
                                        ...
                                        <h3 className={`text-xl font-bold font-heading uppercase tracking-wide ${isPremium ? "text-white" : "text-gray-900"
                                            }`}>
                                            {plan.name}
                                        </h3>
                                    </div>
                                    ...
                                    <div className="flex items-baseline gap-2">
                                        {plan.originalPrice && (
                                            <span className={`text-base line-through font-medium ${isPremium ? "text-zinc-600" : "text-gray-400"}`}>
                                                {plan.originalPrice}
                                            </span>
                                        )}
                                        <span className="text-4xl font-black font-heading tracking-tight">{plan.price}</span>
                                ...
                                {/* Card Body - Analytics Highlights */}
                                <div className={`px-8 py-4 flex-none border-b border-gray-100/50 flex items-center gap-2 ${isPremium ? "bg-zinc-900/60 border-zinc-800" : "bg-primary-light/30"
                                    }`}>
                                    <BarChart3 className={`w-4 h-4 shrink-0 ${isPremium ? "text-accent" : "text-primary"}`} />
                                    <div className="text-xs font-bold leading-relaxed">
                                        <span className={isPremium ? "text-zinc-400" : "text-gray-500"}>Analytics: </span>
                                ...
                                <div className="p-8 flex-1 flex flex-col justify-between space-y-8">
                                    ...
                                    <div className="pt-6">
                                        <Link
                                            href={plan.ctaLink}
                                            className={`w-full py-4 px-6 rounded-xl font-bold text-sm transition-all duration-300 flex items-center justify-center gap-2 shadow-sm ${isPremium
            ...
            <section className="max-w-5xl mx-auto px-6 py-12">
                <div className="bg-gradient-to-br from-primary-dark to-primary p-8 md:p-12 rounded-[2.5rem] shadow-xl text-white relative overflow-hidden">

                    <div className="absolute top-0 right-0 w-64 h-64 bg-white/5 rounded-full -translate-y-1/2 translate-x-1/2 pointer-events-none" />
                    <div className="absolute bottom-0 left-0 w-48 h-48 bg-accent/10 rounded-full translate-y-1/3 -translate-x-1/4 pointer-events-none" />

                    <div className="relative z-10 grid grid-cols-1 lg:grid-cols-5 gap-8 items-center">

                        {/* Finder Left Text */}
                        <div className="lg:col-span-2 space-y-4">
                            <h2 className="text-3xl font-black font-heading leading-tight uppercase">
                                Find the Perfect Plan for You
                            </h2>
                            <p className="text-sm text-white/80 leading-relaxed">
                                Answer three quick questions about your property volume, expected closing speed, and dashboard tracking needs to find your campaign.
                            </p>
                        </div>

                        {/* Finder Interactive Questionnaire */}
                        <div className="lg:col-span-3 bg-white text-gray-900 p-6 md:p-8 rounded-3xl shadow-lg border border-white/10">
                            ...
                            {!finderResult ? (
                                <div className="space-y-6">
                                    ...
                                    <div className="space-y-3">
                                        <label className="text-xs font-black uppercase text-primary tracking-wider flex items-center gap-1.5">
                                            <span className="w-4 h-4 rounded-full bg-primary/10 text-primary flex items-center justify-center font-bold text-[9px]">1</span>
                                            How long do you want the advertisement active?
                                        </label>
                                        <div className="grid grid-cols-3 gap-2">
                                            {[
                                                { id: "short", label: "15-30 Days" },
                                                { id: "medium", label: "3 Months (90 days)" },
                                                { id: "long", label: "6 Months (180 days)" }
                                            ].map((item) => (
                                                <button
                                                    key={item.id}
                                                    onClick={() => setFinderDuration(item.id)}
                                                    className={`py-2.5 px-2 rounded-xl text-xs font-bold transition-all border ${finderDuration === item.id
                                    ...
                                    <div className="space-y-3">
                                        <label className="text-xs font-black uppercase text-primary tracking-wider flex items-center gap-1.5">
                                            <span className="w-4 h-4 rounded-full bg-primary/10 text-primary flex items-center justify-center font-bold text-[9px]">2</span>
                                            What volume of verified leads do you expect?
                                        </label>
                                        <div className="grid grid-cols-3 gap-2">
                                            {[
                                                { id: "low", label: "Low (<3 leads)" },
                                                { id: "moderate", label: "Moderate (30 leads)" },
                                                { id: "high", label: "High / Unlimited" }
                                            ].map((item) => (
                                                <button
                                                    key={item.id}
                                                    onClick={() => setFinderLeads(item.id)}
                                                    className={`py-2.5 px-2 rounded-xl text-xs font-bold transition-all border ${finderLeads === item.id
                                                        ? "bg-primary/10 border-primary text-primary"
                                                        : "border-gray-200 text-gray-600 hover:border-gray-300"
                                                        }`}
                                                >
                                    ...
                                    <div className="space-y-3">
                                        <label className="text-xs font-black uppercase text-primary tracking-wider flex items-center gap-1.5">
                                            <span className="w-4 h-4 rounded-full bg-primary/10 text-primary flex items-center justify-center font-bold text-[9px]">3</span>
                                            Do you need dashboard stats & click tracking?
                                        </label>
                                        <div className="grid grid-cols-3 gap-2">
                                            {[
                                                { id: "none", label: "No / Direct listing only" },
                                                { id: "basic", label: "Basic Clicks & Views" },
                                                { id: "advanced", label: "Demographics & CTR" }
                                            ].map((item) => (
                                                <button
                                                    key={item.id}
                                                    onClick={() => setFinderAnalytics(item.id)}
                                                    className={`py-2.5 px-2 rounded-xl text-xs font-bold transition-all border ${finderAnalytics === item.id
                                                        ? "bg-primary/10 border-primary text-primary"
                                                        : "border-gray-200 text-gray-600 hover:border-gray-300"
                                                        }`}
                                                >
                                    ...
                                    <div className="pt-2">
                                        <button
                                            ...
                                            className={`w-full py-3.5 rounded-xl font-bold text-sm transition-all text-center flex items-center justify-center gap-2 ${finderDuration && finderLeads && finderAnalytics
                                ...
                                >
                                    <div className="w-14 h-14 bg-primary-light text-primary rounded-full flex items-center justify-center mx-auto shadow-sm">
                                        <Sparkles className="w-6 h-6 animate-pulse" />
                                    </div>

                                    <div className="space-y-2">
                                        <p className="text-xs font-black uppercase tracking-wider text-gray-500">We recommend the following package:</p>
                                        <h3 className="text-2xl font-black text-gray-900 uppercase font-heading tracking-wide">
                                            {finderResult === "basic" && "Basic Plan (Free)"}
                                            {finderResult === "boost" && "3-Month Boost"}
                                            {finderResult === "premium" && "6-Month Premium"}
                                        </h3>
                                        <p className="text-sm text-gray-600 max-w-sm mx-auto">
                                            {finderResult === "basic" && "Ideal for a quick trial. Your ad runs for 15-30 days with direct contact details."}
                                            {finderResult === "boost" && "Extends your property visibility for 3 full months (90 days) with views/clicks metrics and directory presence."}
                                            {finderResult === "premium" && "Full 6-month campaign. Features unlimited pre-screened leads, embedded video players, geo-traffic stats, and top comparison suggestions."}
                                        </p>
                                    </div>
                                    ...
                                    <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
                                        ...
                                        <Link
                                            href={finderResult === "basic" ? "/post-property" : finderResult === "boost" ? "/post-property?plan=3months" : "/post-property?plan=6months"}
