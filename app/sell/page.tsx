...
export default function SellPage() {
  return (
      <main className="min-h-screen bg-white pt-20 font-body text-slate-950">
        ...
        <section
            id="owner-plans"
            className="relative overflow-hidden border-y border-slate-200 bg-[#f5f7f6]"
        >
          <div
              className="pointer-events-none absolute -right-48 top-20 h-[440px] w-[440px] rounded-full bg-teal-100/50 blur-3xl"
              aria-hidden="true"
          />
          ...
          <div className="relative mx-auto max-w-7xl px-5 py-20 sm:px-6 lg:px-8 lg:py-24">
            <div className="max-w-3xl">
              ...
              <h2 className="mt-3 font-heading text-3xl font-black tracking-[-0.035em] text-slate-950 sm:text-4xl">
                Start free, then pay only when the extra reach is useful.
              </h2>

              <p className="mt-4 max-w-2xl text-base leading-7 text-slate-600">
                Compare the included listing duration, media limits, visibility
                and analytics before selecting a plan on the pricing page.
              </p>
            </div>
            ...
            <div className="mt-14 grid gap-6 lg:grid-cols-3 lg:items-stretch">
              {OWNER_PLANS.map((plan) => {
                ...
                const planLabel = isGold
                    ? "Recommended for more visibility"
                    : isPlatinum
                        ? "Maximum visibility and insights"
                        : "A simple place to begin";

                const ctaLabel =
                    plan.tier === "silver"
                        ? "View Silver plan"
                        : `Choose ${plan.presentation.displayName}`;

                return (
                    <article
                        key={plan.tier}
                        className={`relative isolate flex h-full flex-col overflow-hidden rounded-[2rem] border p-6 transition duration-300 hover:-translate-y-1 sm:p-7 ${theme.card}`}
                    >
                      {/* Gold visual treatment */}
                      {isGold ? (
                          <>
                            <div
                                className="pointer-events-none absolute inset-x-0 top-0 h-1.5 bg-gradient-to-r from-teal-300 via-primary to-teal-500"
                                aria-hidden="true"
                            />
                      ...
                      {/* Platinum visual treatment */}
                      {isPlatinum ? (
                          <>
                            <div
                                className="pointer-events-none absolute inset-x-0 top-0 h-1.5 bg-gradient-to-r from-teal-400 via-cyan-200 to-teal-400"
                                aria-hidden="true"
                            />
            ...
            <span
                className={`flex h-13 w-13 items-center justify-center rounded-2xl p-3.5 ${theme.iconBox}`}
            >
              <Icon size={23} aria-hidden="true" />
                          ...
                          {plan.presentation.badge ? (
                              <span
                                  className={`rounded-full px-3.5 py-2 text-[10px] font-black uppercase tracking-[0.12em] ${theme.badge}`}
                              >
                {plan.presentation.badge}
              </span>
                          ) : null}
                        ...
                        <p
                            className={`mt-7 text-[10px] font-black uppercase tracking-[0.15em] ${theme.eyebrow}`}
                        >
                          {planLabel}
                        </p>
                        ...
                        <p
                            className={`mt-3 min-h-[4.5rem] text-sm leading-6 ${theme.muted}`}
                        >
                        ...
                        {/* Price panel */}
                        <div
                            className={`mt-6 rounded-2xl border p-4 ${theme.priceBox}`}
                        >
                          <p
                              className={`text-[10px] font-black uppercase tracking-[0.12em] ${theme.muted}`}
                          >
                            Listing plan
                          </p>
                  ...
                  {new Intl.NumberFormat("en-IN", {
                    ...
                    maximumFractionDigits: 0,
                      ...
                      plan.presentation.originalPriceInPaise / 100,
                  )}
                </span>
                              </p>
                          ) : null}
                        ...
                        <div
                            className={`my-7 h-px ${theme.divider}`}
                            aria-hidden="true"
                        />

                        {/* Features */}
                        <p
                            className={`text-[10px] font-black uppercase tracking-[0.13em] ${theme.muted}`}
                        >
                          What is included
                        </p>
                        ...
                        <ul className="mt-4 space-y-3">
                          {features.map((feature) => (
                              <li
                                  key={feature}
                                  className={`flex items-start gap-3 text-sm leading-6 ${theme.feature}`}
                              >
                <span
                    className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full ${theme.check}`}
                >
                  <Check size={12} aria-hidden="true" />
                        ...
                        {/* All plan decisions now go through pricing */}
                        <div className="mt-auto pt-8">
                          <Link
                              href="/pricing"
                              className={`flex min-h-13 w-full items-center justify-center gap-2 rounded-xl px-5 py-3.5 text-sm font-black transition duration-200 ${theme.cta}`}
                          >
                            {ctaLabel}
                          ...
                          {plan.tier !== "silver" ? (
                              <p
                                  className={`mt-3 text-center text-[10px] font-semibold leading-4 ${theme.muted}`}
                              >
                                Review the full plan before continuing
                              </p>
                          ) : (
            ...
            <div className="mt-6 flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-teal-50 text-primary">
                ...
                <div>
                  <p className="font-black text-slate-950">
                    Not ready to choose a paid plan?
                  </p>
                  <p className="mt-1 text-sm text-slate-500">
                    Begin with Silver and create one active listing for 30 days.
                  </p>
                </div>
              </div>
