...
export default function MapPinPicker({ city, latitude, longitude, onChange }: { city: string; latitude: string; longitude: string; onChange: (latitude: string, longitude: string) => void }) {
    ...
    return <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-4">
        ...
        <p className="mt-1 text-xs text-slate-600">Tap the map to mark the location. A precise pin helps buyers find it.</p>
        ...
        {hasPin && <p className="mt-2 text-xs text-slate-600">Pin: {latitude}, {longitude} <button type="button" className="ml-2 font-bold text-red-700 underline" onClick={() => onChange("", "")}>Clear</button></p>}
    </div>;
}
